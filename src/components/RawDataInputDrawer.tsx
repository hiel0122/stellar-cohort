...
import { AlertTriangle, Plus, Trash2, Check, Search, Save, CalendarIcon, RotateCcw, Target, Receipt, ChevronDown } from "lucide-react";
      ...
      >
        <SheetHeader className="px-4 pt-4 pb-2 border-b shrink-0">
          ...
          <SheetDescription className="text-[11px]">
            기수별 원천 데이터 · 비용(L1) · 목표를 통합 관리하세요.
          </SheetDescription>
        </SheetHeader>

        <UnifiedPanel defaultInstructor={defaultInstructor} defaultCourse={defaultCourse} />
      </SheetContent>
    </Sheet>
  );
}
...
// ═══════════════════════ Unified Panel ═══════════════════════
function UnifiedPanel({ defaultInstructor, defaultCourse }: { defaultInstructor?: string; defaultCourse?: string }) {
  ...
  const handleNewCohortCreated = (id: string) => setSelectedId(id);
  ...
  const costsForCohort = useMemo(() => {
    if (!form) return [];
    return getCostsForCohort(form.instructor_name, form.course_title, form.cohort_no);
  }, [form?.instructor_name, form?.course_title, form?.cohort_no, platformCosts]);
  ...
  const [costForm, setCostForm] = useState<PlatformCost | null>(null);
  ...
  // Reset cost selection when cohort changes
  useEffect(() => {
    if (costsForCohort.length > 0) {
      setSelectedCostId(costsForCohort[0].id);
    } else {
  ...
  // Sync cost form
  useEffect(() => {
    if (!selectedCostId) { setCostForm(null); return; }
    const found = costsForCohort.find((c) => c.id === selectedCostId);
    if (found) setCostForm({ ...found });
  }, [selectedCostId, costsForCohort]);

  const costAutoSave = useCallback((updated: PlatformCost) => {
  ...
  const handleCostCreated = (newId: string) => {
    setSelectedCostId(newId);
  };

  const handleDeleteCost = () => {
    if (!costForm) return;
    deletePlatformCost(costForm.id);
    setSelectedCostId(null);
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      ...
      {/* ═══ Left: Cohort List (35%) ═══ */}
      <div className="w-[35%] min-w-0 border-r flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto">
          <Table>
            ...
            <TableBody>
              {filtered.map((c) => {
                ...
                return (
                  <TableRow key={c.id} className={`cursor-pointer border-b border-border/30 transition-colors ${isSelected ? "bg-primary/5" : "hover:bg-muted/30"}`} onClick={() => setSelectedId(c.id)}>
                    ...
                    <TableCell className="py-1.5 px-2 text-[11px] max-w-[100px] truncate">{c.course_title}</TableCell>
              ...
              {filtered.length === 0 && (
                <TableRow><TableCell colSpan={4} className="text-center py-8 text-xs text-muted-foreground">데이터가 없습니다</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
      ...
      <div className="w-[65%] min-w-0 overflow-y-auto overflow-x-hidden">
        {form ? (
          <div className="space-y-0">
            ...
            <div className="sticky top-0 z-10 bg-background border-b px-4 py-2 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-xs font-medium text-foreground truncate">
                  {form.instructor_name} / {form.course_title} / {form.cohort_no}기
                </span>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <Button variant="ghost" size="sm" className="h-6 text-[10px] px-2" onClick={() => costSectionRef.current?.scrollIntoView({ behavior: "smooth" })}>
                  <Receipt className="h-3 w-3 mr-1" /> 비용
                </Button>
                <Button variant="ghost" size="sm" className="h-6 text-[10px] px-2" onClick={() => targetSectionRef.current?.scrollIntoView({ behavior: "smooth" })}>
                  <Target className="h-3 w-3 mr-1" /> 목표
                </Button>
              </div>
            </div>
            ...
            <div className="p-4 space-y-3 border-b">
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-medium">기수 원데이터</p>
            ...
            <div ref={costSectionRef} className="border-b">
              <button
                ...
                onClick={() => setCostSectionOpen((v) => !v)}
              >
                <div className="flex items-center gap-2">
                  <Receipt className="h-3.5 w-3.5 text-muted-foreground" />
                  <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-medium">비용 입력 (L1)</span>
                  {costsForCohort.length > 0 && (
                    <Badge variant="secondary" className="text-[9px] h-4 px-1.5">{costsForCohort.length}건</Badge>
                  )}
                </div>
                <ChevronDown className={cn("h-3.5 w-3.5 text-muted-foreground transition-transform", costSectionOpen && "rotate-180")} />
              </button>
              ...
              {costSectionOpen && (
                <div className="px-4 pb-4 space-y-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    <Button variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={() => setShowNewCostModal(true)}>
                      <Plus className="h-3 w-3" /> 비용 추가
                    </Button>
                    {selectedCostId && (
                      <Button variant="outline" size="sm" className="h-7 text-xs gap-1 text-destructive hover:text-destructive" onClick={handleDeleteCost}>
                        <Trash2 className="h-3 w-3" />
                      </Button>
                    )}
                    {costSaveStatus === "saved" && <Badge variant="secondary" className="text-[9px] h-5 gap-1 ml-auto"><Check className="h-3 w-3" /> 저장됨</Badge>}
                  </div>
                  ...
                  {costsForCohort.length > 0 ? (
                    <>
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow className="border-b border-border/50">
                              ...
                              <TableHead className="h-7 text-[9px] uppercase tracking-widest px-2 text-right">합계</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {costsForCohort.map((c) => {
                              const isSelected = c.id === selectedCostId;
                              return (
                                <TableRow
                                  key={c.id}
                                  data-cost-id={c.id}
                                ...
                                >
                                  ...
                                  <TableCell className="py-1.5 px-2 text-[11px] text-right tabular-nums">{formatWonFull(c.fee_amount)}</TableCell>
                                  <TableCell className="py-1.5 px-2 text-[11px] text-right tabular-nums">{formatWonFull(c.ad_cost_amount)}</TableCell>
                                  <TableCell className="py-1.5 px-2 text-[11px] text-right tabular-nums font-medium">{formatWonFull(c.fee_amount + c.ad_cost_amount)}</TableCell>
                                </TableRow>
                              );
                            })}
                          </TableBody>
                      ...
                      {costForm && (
                        <div className="rounded-md border border-border/60 p-3 space-y-2.5">
                          ...
                          <div className="space-y-1">
                            ...
                            <Input value={costForm.platform_name} onChange={(e) => updateCostField("platform_name", e.target.value)} className="h-8 text-xs w-full" />
                            {recentPlatforms.length > 0 && (
                              <div className="flex flex-wrap gap-1 mt-1">
                                {recentPlatforms.slice(0, 5).map((name) => (
                                  <Button key={name} variant="outline" size="sm" className="h-5 text-[9px] px-1.5" onClick={() => updateCostField("platform_name", name)}>
                                    {name}
                                  </Button>
                                ))}
                              </div>
                          ...
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              value={costForm.fee_rate_pct ?? ""}
                              onChange={(e) => {
                                const r = Number(e.target.value) || 0;
                                const calcFee = Math.round((form?.revenue ?? 0) * (r / 100));
                                updateCostField("fee_rate_pct", r);
                                updateCostField("fee_amount", calcFee);
                              }}
                              className="tabular-nums h-8 text-xs w-full"
                            />
                            <p className="text-[10px] text-muted-foreground">
                              수수료 금액 = <span className="font-medium text-foreground tabular-nums">{(costForm.fee_amount ?? 0).toLocaleString("ko-KR")}원</span>
                              <span className="ml-1">(매출 {formatWonFull(form?.revenue ?? 0)} × {costForm.fee_rate_pct ?? 0}%)</span>
                            </p>
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">광고비 (원)</Label>
                            <Input
                              value={costForm.ad_cost_amount ? fmtInput(costForm.ad_cost_amount) : ""}
                              onChange={(e) => { const n = parseNum(e.target.value); if (n >= 0) updateCostField("ad_cost_amount", n); }}
                              className="tabular-nums h-8 text-xs w-full" inputMode="numeric"
                            />
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">메모</Label>
                            <Input value={costForm.note} onChange={(e) => updateCostField("note", e.target.value)} className="h-8 text-xs w-full" placeholder="(선택)" />
                          </div>
                        </div>
                      )}
                      ...
                      {/* Cost summary */}
                      <div className="rounded-md bg-muted p-2.5 space-y-1">
                        <p className="text-[9px] uppercase tracking-widest text-muted-foreground mb-1">비용 요약</p>
                        <div className="flex justify-between text-xs"><span className="text-muted-foreground">수수료 합계</span><span className="font-medium tabular-nums">{formatWonFull(totalFee)}</span></div>
                    ...
                    <div className="text-center py-4">
                      <p className="text-xs text-muted-foreground">등록된 비용이 없습니다</p>
                      <p className="text-[10px] text-muted-foreground mt-1">"비용 추가" 버튼을 눌러 플랫폼별 비용을 입력하세요</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div ref={targetSectionRef}>
              <button
                className="w-full flex items-center justify-between px-4 py-3 hover:bg-muted/30 transition-colors"
                onClick={() => setTargetSectionOpen((v) => !v)}
              >
                <div className="flex items-center gap-2">
                  <Target className="h-3.5 w-3.5 text-muted-foreground" />
                  <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-medium">기수 목표</span>
                </div>
                <ChevronDown className={cn("h-3.5 w-3.5 text-muted-foreground transition-transform", targetSectionOpen && "rotate-180")} />
              </button>
...
function InlineTargetsSection({ instructorName, courseTitle, cohortNo }: { instructorName: string; courseTitle: string; cohortNo: number }) {
  ...
  return (
    <div className="space-y-2.5">
      {!targets && (
        <p className="text-[10px] text-muted-foreground">목표 미설정 — 아래에서 입력하세요</p>
      )}
